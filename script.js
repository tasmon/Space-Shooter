...
let player, bullets, enemies, powerUps, enemyBullets, bossBullets, lives, score, level;
let highScore = 0;
...
let stars = [];
for (let i = 0; i < 40; i++) {
  stars.push({ x: Math.random() * 240, y: Math.random() * 320 });
}
let bgHue = 220;
...
function startGame() {
  ...
  resetGame();
  gameInterval = setInterval(updateGame, 60); // smoother tick
}
...
function updateGame() {
  ...
  if (boss) {
    ...
    // Boss shooting frequency scales with level
    let shootChance = 0.01 + (level * 0.002);
    if (Math.random() < shootChance) {
      bossBullets.push({ x: boss.x + boss.width / 2, y: boss.y + boss.height, speed: 2 + level * 0.1 });
      if (level >= 10) {
        bossBullets.push({ x: boss.x + 20, y: boss.y + boss.height, speed: 2 + level * 0.1 });
        bossBullets.push({ x: boss.x + boss.width - 20, y: boss.y + boss.height, speed: 2 + level * 0.1 });
      }
    }
    ...
    bossBullets.forEach((bb, i) => {
      bb.y += bb.speed;
      ctx.fillRect(bb.x, bb.y, 6, 10);
      if (bb.y > 320) bossBullets.splice(i, 1);
      if (bb.x > player.x && bb.x < player.x + player.width &&
          bb.y > player.y && bb.y < player.y + player.height) {
  ...
  enemies.forEach((e, i) => {
    ...
    if (Math.random() < 0.005) {
      enemyBullets.push({ x: e.x, y: e.y, speed: 2 });
    }
 ...
 // HUD
  ...
  ctx.textAlign = "right";
  ctx.fillText("Lives: " + lives, 235, 15);
  ctx.fillText("High: " + highScore, 235, 30);

  if (boss) {
    ctx.textAlign = "center";
    ctx.fillText("Boss HP: " + boss.hp, 120, 45);
  }
}
